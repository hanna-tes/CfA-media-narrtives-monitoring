...
.stChatMessage > div:first-child {{
    ...
    margin-right: 0.75rem; /* Space between avatar and message */
}}
...
/* Style the message content container */
...
st.header("💬 Ask Insights from the Dashboard Data") # Changed emoji to speech bubble
...
if 'search_engine' in st.session_state:
    ...
    with st.container():
        ...
        for message in st.session_state.messages:
            with st.chat_message(message["role"], avatar=BRIGHT_LOGO_URL): # Use CfA logo as avatar
        ...
        if prompt := st.chat_input("Ask a question about the data..."):
            ...
            with st.chat_message("user", avatar=BRIGHT_LOGO_URL): # Use CfA logo as avatar
                st.markdown(prompt)

            # Display assistant response in chat message container
            with st.chat_message("assistant", avatar=BRIGHT_LOGO_URL): # Use CfA logo as avatar
